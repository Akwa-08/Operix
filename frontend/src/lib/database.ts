    ...
    async getPayrollRecords(periodId: string) {
      const { data, error } = await supabase
        ...
        .select(
          `
          *,
          employee:employee_id(id, employee_code, full_name, position, base_hourly_rate)
        `,
        )
        .eq("payroll_period_id", periodId)
        .order("created_at");
    ...
    async computePayroll(periodId: string) {
      ...
      const { data: logs, error } = await supabase
        ...
        .select(
          ...
          *,
          employee:employee_id(
            id, employee_code, full_name, base_hourly_rate,
            philhealth_contribution, hdmf_contribution
          )
        `,
        )
      ...
      for (const log of logs || []) {
        ...
        const ignoreCarryOverNames = ["ROBYMIE", "EJ JUMADAS", "DANIEL"];
        const carryOverFromPrevious = ignoreCarryOverNames.includes(emp.full_name?.toUpperCase()) ? 0 : (carryOverByEmp[emp.id] || 0);
