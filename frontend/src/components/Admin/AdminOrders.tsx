...
<KpiCard title="Total Orders"   value={stats.total}          icon={<Package size={16}     />} iconColor="text-cyan-600" 
          onClick={() => {
            setStatusFilter("All");
            setBreakdown({ title: "Total Orders", description: "All orders in the system.", data: orders, columns: orderColumns, icon: <Package size={24} /> });
          }} />
        <KpiCard title="Active"         value={activeCount}           icon={<Clock size={16}       />} iconColor="text-purple-600" 
          onClick={() => {
            setStatusFilter("Active");
            setBreakdown({ title: "Active Orders", description: "Orders currently in Designing, Payment, or Production.", data: orders.filter((o) => ["Designing", "Payment", "Production"].includes(o.status)), columns: orderColumns, icon: <Clock size={24} /> });
          }} />
        <KpiCard title="Ready Pickup"   value={stats.readyPickup}    icon={<CheckCircle size={16} />} iconColor="text-green-600" 
          ...
          onClick={() => {
            setStatusFilter("Ready Pickup");
            setBreakdown({ title: "Ready for Pickup", description: "Orders awaiting customer pickup.", data: orders.filter(o => o.status === "Pickup"), columns: orderColumns, icon: <CheckCircle size={24} /> });
          }} />
        <KpiCard title="Unpaid"         value={stats.completedUnpaid} icon={<DollarSign size={16} />} iconColor="text-orange-600"
          ...
          onClick={() => {
            ...
            setBreakdown({ title: "Unpaid Orders", description: "Orders with pending or partial payments.", data: orders.filter(o => o.paymentStatus !== "Paid" && (o.status as string) !== "Cancelled"), columns: orderColumns, icon: <DollarSign size={24} /> });
          }} />
        <KpiCard title="Overdue"        value={stats.overdue}        icon={<AlertCircle size={16} />} iconColor="text-red-600"
          ...
          onClick={() => {
            ...
            setBreakdown({ title: "Overdue Orders", description: "Orders past their due date.", data: orders.filter((o) => o.dueDate && new Date(o.dueDate) < new Date() && !["Completed", "Cancelled", "Pickup"].includes(o.status as string)), columns: orderColumns, icon: <AlertCircle size={24} /> });
          }} />
      </div>
